...
async function captureText(page, selector) {
  try {
    const el = await page.$(selector);
    if (!el) return null;
...
async function testTimelessToons(browser) {
  ...
  try {
    // Homepage
    await page.goto('https://timelesstoons.org', { waitUntil: 'networkidle', timeout: 30000 });
    ...
    // Search page
    await page.goto('https://timelesstoons.org/search_series', { waitUntil: 'networkidle', timeout: 30000 });
    ...
    // Try to find a manga link and click
...
async function testGenzToons(browser) {
  ...
  try {
    // Homepage
    await page.goto('https://genztoons.org', { waitUntil: 'networkidle', timeout: 30000 });
    ...
    // Search page
    await page.goto('https://genztoons.org/search_series', { waitUntil: 'networkidle', timeout: 30000 });
...
async function testStoneScape(browser) {
  ...
  try {
    // Homepage
    await page.goto('https://stonescape.xyz', { waitUntil: 'networkidle', timeout: 30000 });
    ...
    console.log('Search API saved, status:', searchRes.status, 'body:', searchRes.body.slice(0, 200));

    // If search has results, try detail
    let searchData;
    ...
    if (searchData?.data?.length > 0) {
      ...
      // Also navigate to the web page for screenshot
      await page.goto(`https://stonescape.xyz/series/${slug}`, { waitUntil: 'networkidle', timeout: 30000 });
...
async function testWuxiaWorld(browser) {
  ...
  try {
    // Homepage
    await page.goto('https://wuxiaworld.site', { waitUntil: 'networkidle', timeout: 30000 });
    ...
    // Search page
    await page.goto('https://wuxiaworld.site/?s=naruto&post_type=wp-manga', { waitUntil: 'networkidle', timeout: 30000 });
    ...
    // Try to find a manga/novel link and click
...
(async () => {
  ...
  console.log('\n=== All results saved to batch7_raw_results.json ===');
