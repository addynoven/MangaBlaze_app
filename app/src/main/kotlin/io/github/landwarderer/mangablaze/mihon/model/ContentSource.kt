...
import io.github.landwarderer.mangablaze.core.util.ext.toLocaleOrNull
...
import io.github.landwarderer.mangablaze.mihon.parsers.util.splitTwoParts
...
fun contentSource(name: String?): ContentSource {
	...
	// GlobalExtensionManager.mangaSources.value.find { it.name == name }?.let { return io.github.landwarderer.mangablaze.core.parser.kotatsu.KotatsuParserSource(it) }
...
fun ContentSource.getOriginLabel(context: Context): String? = when (this) {
	...
	is io.github.landwarderer.mangablaze.mihon.model.MihonMangaSource -> "Mihon"
	else -> null
}
...
fun ContentSource.getTitle(context: Context): String {
	val baseTitle = when (val source = unwrap()) {
		...
		is io.github.landwarderer.mangablaze.mihon.model.MihonMangaSource -> source.displayName
		else -> {
	...
	get() {
		...
		return when (unwrapped) {
			is KotatsuParserSource -> unwrapped.isBroken
				...
				// io.github.landwarderer.mangablaze.core.extensions.GlobalExtensionManager.contentSources.value.find { it.originalSource == unwrapped || it.name == unwrapped.name }?.isBroken == true ||
				// io.github.landwarderer.mangablaze.core.extensions.GlobalExtensionManager.mangaSources.value.find { it.originalSource == unwrapped || it.name == unwrapped.name }?.isBroken == true
				false
			}
		}
	}
