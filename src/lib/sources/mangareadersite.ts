...
const BASE_URL = 'https://mangareader.site'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`MangaReader.site fetch error: ${res.status} ${url}`)
  const html = await res.text()
  return cheerio.load(html)
}

export const mangareadersiteSource: MangaSource = {
  id: 'mangareadersite',
