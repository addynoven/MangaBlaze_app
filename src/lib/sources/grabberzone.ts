...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`GrabberZone fetch error: ${res.status} ${url}`)
...
export const grabberzoneSource: MangaSource = {
  id: 'grabberzone',
  name: 'Grabber Zone',
  type: 'scraper',
  ...
  async search(query: string, limit = 20): Promise<SourceManga[]> {
    try {
      ...
      $('.c-tabs-item').each((_, el) => {
        const item = $(el)
        const link = item.find('.post-title h3 a, .post-title h4 a').first()
  ...
  async getChapters(
    ...
    try {
      ...
      $('li.wp-manga-chapter a').each((_, el) => {
        const link = $(el)
        const href = link.attr('href') || ''
        const slug = extractChapterSlugFromHref(href)
        if (!slug || seen.has(slug)) return
        seen.add(slug)

        const titleText = link.text().trim()
        const match = titleText.match(/Chapter\s+([\d.]+)/i) || slug.match(/([\d.]+)/)
