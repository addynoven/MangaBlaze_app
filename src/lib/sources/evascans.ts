...
const BASE_URL = 'https://evascans.org'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`Eva Scans fetch error: ${res.status} ${url}`)
...
export const evascansSource: MangaSource = {
  id: 'evascans',
  name: 'Eva Scans',
  type: 'scraper',
  ...
  async search(query: string, limit = 20): Promise<SourceManga[]> {
    try {
      ...
      $('.manga-card-v').each((_, el) => {
        const item = $(el)
        const link = item.find('.card-v-title a').first()
        ...
        const cover =
          item.find('.card-v-cover img').attr('src') ||
          item.find('.card-v-cover img').attr('data-src') ||
  ...
  async getManga(mangaId: string): Promise<SourceMangaDetail | null> {
    try {
      ...
      $('.imptdt i, .post-status .summary-content').each((_, el) => {
    ...
    try {
      ...
      $('#chapterlist .chlink, .eplister .chlink').each((_, el) => {
        ...
        chapters.push({
          ...
          externalUrl: href?.startsWith('http') ? href : `${BASE_URL}/series/${mangaId}/${id}/`,
          isUnavailable: false,
        })
      })
