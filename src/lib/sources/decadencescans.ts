...
const BASE_URL = 'https://reader.decadencescans.com'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`Decadence Scans fetch error: ${res.status} ${url}`)
...
function extractChapterNumberFromHref(href: string): string {
  const match = href.match(/\/chapter-(\d+(?:\.\d+)?)\/?$/)
  if (match) return match[1]
  const extraMatch = href.match(/\/extra_(\d+)\/?$/)
  if (extraMatch) return `extra-${extraMatch[1]}`
  return '?'
}

export const decadencescansSource: MangaSource = {
  id: 'decadencescans',
  ...
  async getChapters(
    ...
    try {
      ...
      $('li.wp-manga-chapter a').each((_, el) => {
        ...
        const volumeMatch = href.match(/\/volume-(\d+)\//)
