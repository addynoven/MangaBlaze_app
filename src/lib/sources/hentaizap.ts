...
const BASE_URL = 'https://hentaizap.com'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`HentaiZap fetch error: ${res.status} ${url}`)
  return cheerio.load(await res.text())
}

export const hentaizapSource: MangaSource = {
  id: 'hentaizap',
  name: 'HentaiZap',
  type: 'scraper',
  ...
  async search(query: string, limit = 20): Promise<SourceManga[]> {
    try {
      ...
      $('.thumb').each((_, el) => {
        const thumb = $(el)
        const link = thumb.find('h2 a').first()
  ...
  async getManga(mangaId: string): Promise<SourceMangaDetail | null> {
    try {
      ...
      const title = $('title').first().text().trim().replace(/ - HentaiZap$/, '')
      if (!title) return null

      const cover =
        $('.gallery_page img.lazy').attr('data-src') ||
        $('.gallery_page img.lazy').attr('src') ||
        '/images/placeholder.png'

      const pagesText = $('input#load_pages').attr('value') || $('span.info_pg').text()
  ...
  async getChapterPages(chapterId: string): Promise<SourcePage[]> {
    try {
      ...
      const firstImg = first$('img#fimg').attr('data-src') || first$('img#fimg').attr('src')
      if (!firstImg) return []

      // Derive pattern: replace /1.webp or /1.jpg with /{n}.ext
      ...
      const pagesText = gallery$('input#load_pages').attr('value') || gallery$('span.info_pg').text()
      const pagesMatch = pagesText.match(/(\d+)/)
