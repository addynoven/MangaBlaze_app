...
const BASE_URL = 'https://hentaixcomic.com'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`HentaiXComic fetch error: ${res.status} ${url}`)
...
export const hentaixcomicSource: MangaSource = {
  id: 'hentaixcomic',
  name: 'HentaiXComic',
  type: 'scraper',
  ...
  async getChapters(
    ...
    try {
      ...
      // Fallback for AJAX-loaded chapters (init-links)
      if (chapters.length === 0) {
        $('#init-links a, #btn-read-first, #btn-read-last').each((_, el) => {
          const link = $(el)
          const href = link.attr('href') || ''
          const slugMatch = href.match(/\/manga\/[^/]+\/([^/]+)\/?$/)
          const id = slugMatch?.[1] || href
