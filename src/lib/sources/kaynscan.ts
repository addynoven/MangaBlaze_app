...
const BASE_URL = 'https://kaynscan.com'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`KaynScan fetch error: ${res.status} ${url}`)
  const html = await res.text()
  return cheerio.load(html)
}

function stripSiteSuffix(description: string): string {
  // Descriptions end with " - Kayn Scans - Kaynscans.com Read English Manhwa Releases..."
  const idx = description.indexOf(' - Kayn Scan')
  return idx > 0 ? description.slice(0, idx).trim() : description.trim()
}

export const kaynscanSource: MangaSource = {
  id: 'kaynscan',
  name: 'Kayn Scans',
  type: 'scraper',
