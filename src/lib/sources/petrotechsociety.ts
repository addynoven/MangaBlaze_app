...
const BASE_URL = 'https://www.petrotechsociety.org'
const USER_AGENT =
...
export const petrotechsocietySource: MangaSource = {
  id: 'petrotechsociety',
  ...
  async getChapters(
    ...
    try {
      ...
      $('li.wp-manga-chapter a').each((_, el) => {
        const link = $(el)
        const href = link.attr('href') || ''
        const mangaSlug = extractSlugFromUrl(href)
        const chapterMatch = href.match(/\/manga\/[^/]+\/([^/]+)\/$/)
        const chapterSlug = chapterMatch?.[1] || ''
        if (!chapterSlug || !mangaSlug) return

        const id = `${mangaSlug}/${chapterSlug}`
        if (chapters.some((c) => c.id === id)) return

        const titleText = link.text().trim()
        const numMatch = titleText.match(/Chapter\s+([\d.]+)/i) || chapterSlug.match(/([\d.]+)/)
