...
export const mangaheSource: MangaSource = {
  id: 'mangahe',
  name: 'MangaHe',
  type: 'scraper',
  ...
  async search(query: string, limit = 20): Promise<SourceManga[]> {
    try {
      const url = `${BASE_URL}/?post_type=wp-manga&s=${encodeURIComponent(query)}`
      const $ = await fetchHTML(url)

      const results: SourceManga[] = []
      $('.page-item-detail.manga, .c-tabs-item, .tab-content-wrap .row .col-6').each((_, el) => {
        const item = $(el)
        const link = item.find('.post-title h3 a, .post-title a, h3 a').first()
        ...
        const cover =
          item.find('.item-thumb img, .tab-thumb img, img.img-responsive').first().attr('src') ||
