...
export const rackusreadsSource: MangaSource = {
  id: 'rackusreads',
  name: 'Rackus Comics',
  type: 'scraper',
  ...
  async search(query: string, limit = 20): Promise<SourceManga[]> {
    try {
      ...
      $('a.series').each((_, el) => {
        ...
        const cover =
          link.closest('.bsx, .bs, li, .page-item-detail').find('img').first().attr('src') ||
  ...
  async getManga(mangaId: string): Promise<SourceMangaDetail | null> {
    try {
      ...
      const description = $('.entry-content p, .summary__content p').first().text().trim()
  ...
  async getChapterPages(chapterId: string, mangaId?: string): Promise<SourcePage[]> {
    try {
      ...
      $('.entry-content img, .reading-content img').each((index, el) => {
