...
const BASE_URL = 'https://s2manga.com'
const USER_AGENT =
...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`S2Manga fetch error: ${res.status} ${url}`)
...
export const s2mangaSource: MangaSource = {
  id: 's2manga',
  name: 'S2Manga',
  type: 'scraper',
