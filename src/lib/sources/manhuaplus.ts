...
async function fetchHTML(url: string): Promise<cheerio.CheerioAPI> {
  ...
  if (!res.ok) throw new Error(`ManhuaPlus fetch error: ${res.status} ${url}`)
...
export const manhuaplusSource: MangaSource = {
  id: 'manhuaplus',
  name: 'Manhua Plus',
  type: 'scraper',
  ...
  async search(query: string, limit = 20): Promise<SourceManga[]> {
    try {
      ...
      $('.tab-content-wrap .c-tabs-item').each((_, el) => {
        ...
        const cover =
          link.find('img').attr('data-src') || link.find('img').attr('src') || '/images/placeholder.png'
  ...
  async getChapterPages(chapterId: string, mangaId?: string): Promise<SourcePage[]> {
    try {
      ...
      $('img[decoding="async"]').each((index, el) => {
        ...
        if (src && src.includes('cdn.manhuaplus.com')) {
