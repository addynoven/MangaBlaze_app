...
import { addSearchToHistory, clearSearchHistory } from '@/store/slices/library/librarySlice'
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      fetch(`/api/manga/suggest?q=${encodeURIComponent(keyword)}&source=${suggestSource}`)
        .then(res => res.json())
          ...
          <div className="component">
                  ...
                  <Link href="/updates">
                    ...
                    {unreadUpdates.length > 0 && (
                      <span 
                        ...
                        style={{ top: 10, right: -10, fontSize: 10, padding: '4px 6px' }}
                      >
            ...
            <div id="nav-search" className={classNames(openSearch && 'active')}>
              ...
              <div className="search-inner">
                <form action="/filter" autoComplete="off" onSubmit={handleSearchSubmit}>
                ...
                {openSearch && (keyword.length > 0 || searchHistory.length > 0) && (
                  <div className="suggestion active" style={{ display: 'block', background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '0 0 8px 8px', overflow: 'hidden' }}>
                    ...
                    {keyword.length === 0 && searchHistory.length > 0 && (
                      <div className="history-section">
                        <div className="p-2 small text-muted uppercase fw-bold border-bottom border-white/5 d-flex justify-content-between">
                          Recent Searches
                          <button className="btn btn-link btn-sm p-0 text-muted" onClick={() => dispatch(clearSearchHistory())}>Clear</button>
                        </div>
                    ...
                    {suggestions.length > 0 && (
                      <div className="matches-section">
                        <div className="p-2 small text-muted uppercase fw-bold border-bottom border-white/5">Instant Matches</div>
                        {suggestions.map((item) => (
                          <Link 
                            ...
                            onClick={handleClose}
                          >
                            <img src={item.cover} alt="" style={{ width: 40, height: 50, objectFit: 'cover', borderRadius: 4, marginRight: 12 }} />
                            <div className="text-truncate">
                              <div className="fw-bold">{item.title}</div>
                              <div className="small text-muted">{item.source.toUpperCase()}</div>
                            </div>
                          </Link>
                        ))}
                      </div>
